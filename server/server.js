...
app.post('/api/deleteAccount/:uid', async (req, res) => {
    ...
    if (uid != req.params['uid'])
        return res.json({ "error": 'Not logged into the correct account' });
    ...
    let user = users[0].user;
    if (user.session != session)
        return res.json({ "error": 'Session expired or incorrect' });
    // begin account deletion
    let lists = user.personalLists;
    let images = [user.profPic, user.topPic];
    db.deleteUser(uid);
